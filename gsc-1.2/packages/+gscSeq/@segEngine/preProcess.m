  ...
  case {'gmm_bs','gmm_bs_mixtured'}
    switch(opts.featureSpace)
      case 'rgb'
        ...
        obj.opts.trainFeatures = gscSeq.extractPixels(img);
      case 'rgb_xy'
        obj.features=gscSeq.extractPixels(img);
      ...
      case 'moreImages'
        ...
        obj.opts.trainFeatures = gscSeq.extractPixels(img);
        %size( obj.features )
        for i = 1:length(opts.extraImages),
            obj.opts.trainFeatures = [ obj.opts.trainFeatures gscSeq.extractPixels( im2double(opts.extraImages{i} ) ) ];
            %size( obj.features )
        end
      otherwise
...
gamma=opts.gcGamma; %150

%gamma = 500;%300;%750;%1500;
...
switch(opts.gcNbrType)
  case {'colourGradient','colourGradient8'}
    ...
    %sigma = 0.2381;
   % edgeWeights = exp(-(1/(sigma*2))*(colorWeights));
